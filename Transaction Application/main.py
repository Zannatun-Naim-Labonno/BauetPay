...
class Front:
    ...
    def login(self):
        ...
        self.master.geometry('900x900+300+100')
        self.master.title('BAUETpay')
        self.master.config(bg="cadet blue")
        self.frame = Frame(self.master,bg="cadet blue")
        self.menu_count = 0
        self.logo = PhotoImage(file=r'C:\Users\Israt Ishtiaque\Desktop\CSE 4127-System Analysis & Design\Transaction Application\BAUETpay.png').subsample(4,3)

        self.frame_header = ttk.Frame(self.master)
        self.frame_header.pack()

        ttk.Label(self.frame_header, image=self.logo).grid(
            row=0, column=2, rowspan=6)
        ttk.Label(self.frame_header, text='Welcome to BAUETpay!').grid(
        ...
        ttk.Label(self.frame_body, text='User Name:').grid(
            row=0, column=1, padx=16, pady=16, sticky='sw')
        ttk.Label(self.frame_body, text='Password:').grid(
            row=1, column=1, padx=16, pady=16, sticky='sw')
        self.user = ttk.Entry(self.frame_body, width=44, font=('Arial', 10))
        self.psd = ttk.Entry(self.frame_body, width=44, font=('Arial', 10))
        ...
        def temp_action():
            ...
            if self.log_name == self.vname and self.log_pass == self.vpass:
                messagebox.showinfo(
                    title='Welcome to BAUETpay', message='Welcome {}'.format(self.log_name))
                self.home()
            else:
    ...
    def forgot(self):
        ...
        self.master.geometry('900x720+300+100')
        self.frame_body.pack_forget()
        self.frame_body = ttk.Frame(self.master)
        self.frame_body.pack()
        self.frame_body.config(relief=RIDGE, padding=(30, 15))
    ...
    def home(self):
        ...
        ttk.Label(self.frame_body, wraplength=300, text="Welcome to BAUETpay. This is a simple, fast and elegant way of transferring money to anyone swiftly. Explore for more features.").grid(
    ...
    def create(self):
        ...
        self.bankoptions = ['Trust Bank', 'First Security Islami Bank Limited']
    ...
    def temp_action2(self):
        ...
        if not pattern:
            ...
            if (self.obj.insert_record(self.vname, self.vemail, self.vdob, self.vphone, self.vaddress, self.vfirstpass, self.vupi, self.vbank, self.vacc)) == 1:
                ...
                messagebox.showinfo(
                    title='Welcome to BAUETpay', message='Welcome {}'.format(self.vname))
                self.uid, self.vname, self.vemail, self.vaddress, self.vdob, self.vphone, self.vpass, self.vupi, self.vrecent, self.profile_photo, self.ouid, self.vbank, self.vacc, self.vbal = self.obj.get_details(
                    self.vname)
                self.profile_photo = self.obj.insert_blob(self.imgobj)
                self.home()
    ...
    def transaction_history_friend(self, value):
        ...
        for j, i in enumerate(self.vals):
            if str(i[0]) == self.vacc and str(i[1]) == facc:
                self.message = f'Sent to {i[5]} Taka. {i[2]} on {i[3][:19]}'
                ...
                self.message = f'Received From {i[4]} Taka. {i[2]} on {i[3]}'
